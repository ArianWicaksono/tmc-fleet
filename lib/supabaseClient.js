...
export async function updateTicketStatus(id, nextStatus, note = '') {
  ...
  if (error) throw error;
  await logEvent(id, nextStatus, note || 'Status diperbarui');
  return data;
}

export async function updateTicket(id, fields, note = 'Detail tiket diperbarui') {
  const updates = {
    ...fields,
    updated_at: new Date().toISOString(),
  };
  ...
  if (error) throw error;
  await logEvent(id, data.status, note);
  return data;
}
